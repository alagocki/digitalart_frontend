...
const SubnaviBackendImages = (props) => {

    const [subType] = useState(props.sub);

    const createLink = (target) => {
        const href = "/backend/image/" + target.string;
        return (
            <Link to={href}
                    ...
                    <nav className="bg-transparent md:flex md:items-center flex md:justify-between">
                        <ul className="md:flex md:items-center z-[-1] md:z-auto md:static absolute w-full left-0 md:w-auto md:py-0 py-0 md:pl-0 pl-7 md:opacity-100 opacity-0 top-[-400px] transition-all ease-in duration-500">
                            {(UserUtils.isSuperUser()) ?
                                <li className={(getCurrentPath().includes('image/upload')) ? "md:my-0 p-0 ml-0.5 flex text-blue-500 font-medium" : "md:my-0 p-0 ml-0.5 flex"}>
                                    {createLink({string: "upload"})}
                                </li> : null}
                            <li className={(getCurrentPath().includes('image/list')) ? "md:my-0 p-0 ml-0.5 flex text-blue-500 font-medium" : "md:my-0 p-0 ml-0.5 flex"}>
                                {createLink({string: "list"})}
                            </li>
                            {(undefined !== getSubType() && null !== getSubType()) ?
                                <li className={(getCurrentPath().includes('image/detail')) ? "md:my-0 p-0 ml-0.5 flex text-blue-500 font-medium " : "md:my-0 p-0 ml-0.5 flex"}>
                                    <span
...
export default SubnaviBackendImages;